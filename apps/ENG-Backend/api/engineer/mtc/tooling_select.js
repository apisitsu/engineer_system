...
// ─────────────────────────────────────────────────────────────────────────────
// RULES CRUD  /api/tooling-select/rules
// ─────────────────────────────────────────────────────────────────────────────

// GET /api/tooling-select/rules  – ดึงกฎทั้งหมด
router.get('/rules', async (req, res) => {
  try {
    const r = await engPool.query(
      `SELECT * FROM mtc_selection_rules ORDER BY machine_name, tool_category, id`
    );
    res.json({ success: true, rules: r.rows });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

// POST /api/tooling-select/rules  – เพิ่มกฎใหม่
router.post('/rules', async (req, res) => {
  const { machine_name, tool_category, calc_context, target_tool_table,
          machine_ok_condition, dims, result_fields } = req.body;
  if (!machine_name || !tool_category || !target_tool_table)
    return res.status(400).json({ success: false, error: 'machine_name, tool_category, target_tool_table are required' });
  try {
    const r = await engPool.query(
      `INSERT INTO mtc_selection_rules
      ...
      [machine_name, tool_category, calc_context || null, target_tool_table,
       machine_ok_condition || null,
       JSON.stringify(dims || []), JSON.stringify(result_fields || [])]
    );
    res.json({ success: true, rule: r.rows[0] });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

// PUT /api/tooling-select/rules/:id  – อัปเดตกฎ
router.put('/rules/:id', async (req, res) => {
  const { id } = req.params;
  const { machine_name, tool_category, calc_context, target_tool_table,
          machine_ok_condition, dims, result_fields, is_active } = req.body;
  try {
    const r = await engPool.query(
      `UPDATE mtc_selection_rules SET
         machine_name=$1, tool_category=$2, calc_context=$3, target_tool_table=$4,
         machine_ok_condition=$5, dims=$6, result_fields=$7, is_active=$8,
         updated_at=NOW()
    ...
    if (r.rowCount === 0) return res.status(404).json({ success: false, error: 'Rule not found' });
    res.json({ success: true, rule: r.rows[0] });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
});

// PATCH /api/tooling-select/rules/:id/toggle  – toggle is_active
router.patch('/rules/:id/toggle', async (req, res) => {
  const { id } = req.params;
  try {
    const r = await engPool.query(
      `UPDATE mtc_selection_rules SET is_active = NOT is_active, updated_at=NOW()
...
router.delete('/rules/:id', async (req, res) => {
  ...
  try {
    const r = await engPool.query(
      `DELETE FROM mtc_selection_rules WHERE id=$1 RETURNING id`, [id]
    );
    if (r.rowCount === 0) return res.status(404).json({ success: false, error: 'Rule not found' });
    res.json({ success: true, deletedId: r.rows[0].id });
  } catch (err) { res.status(500).json({ success: false, error: err.message }); }
