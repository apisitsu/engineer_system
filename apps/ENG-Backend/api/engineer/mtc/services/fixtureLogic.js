...
async function findFixtures(cnNumber) {
  try {
    ...
    // Legacy Fallback Logic (if dynamic formulas not set up yet)
    const calc        = calculateToolingParams(partData);
    const ks400b_calc = dynamicKS400B.error ? calculateKS400B_Params(partData) : dynamicKS400B;
    const ks03a_calc  = dynamicKS03A.error ? calculateKS03A_Params(partData) : dynamicKS03A;
    const ks500rd_calc= dynamicKS500RD.error ? calculateKS500RD_Params(partData) : dynamicKS500RD;
    const ks400b5_calc= dynamicKS400B5.error ? calculateKS400B5_Params(partData) : dynamicKS400B5;
    const ks400b6_calc= dynamicKS400B6.error ? calculateKS400B6_Params(partData) : dynamicKS400B6;
