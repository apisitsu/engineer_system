...
**Date:** 2026-04-27  
**Status:** ✅ Phase 1–4 Complete (All Phases)
...
| A2.8 | 🆕 **Cross-Project Data Leakage** | `kanban_acl.js`, `kanban_card.js`, `kanban_board.js`, `websocket.js` | Verify that every data-returning endpoint enforces project-scope isolation. Check that board/card/label/member queries always include a `project_id` constraint. Inspect WebSocket rooms for broadcast leakage between projects. Audit shared board scenarios for information bleed (e.g., labels, members, attachments from Project A visible in Project B) |
| A2.9 | 🆕 **Unit Test Coverage Assessment** | `kanban_workload_calculator.js`, `positionHelper.js`, `kanban_acl.js`, `reportHelpers.js` | Identify whether test files exist for these high-complexity logic modules. Flag critical algorithms (5:3:2 workload, position rebalancing, recursive CTE logic, ACL decision trees) that lack automated test coverage, increasing regression risk during future refactors |
...
| C1.7 | 🆕 **Memory Leak Detection** | `KanbanMain.jsx`, `CardDetailDrawer.jsx`, `BoardView.jsx`, `WorkloadDashboard.jsx` | Audit all `useEffect` hooks for missing cleanup functions — especially `addEventListener` / `removeEventListener` pairs, WebSocket/Socket.io subscriptions (`socket.on` without `socket.off`), `setInterval`/`setTimeout` without `clearInterval`/`clearTimeout`, Zustand `subscribe()` calls without `unsubscribe`, and any `ResizeObserver`/`IntersectionObserver` instances not disconnected on unmount |
...
|:-----:|:------|:------|:----------------|
| **1** | Backend Core Logic | `kanban_card.js`, `kanban_project.js`, `kanban_acl.js`, `positionHelper.js`, `websocket.js` | A2 (incl. A2.8 Data Leakage, A2.9 Test Coverage), B5, D1-D3, D5 |
...
| **3** | Frontend Core & State | `KanbanMain.jsx`, `CardDetailDrawer.jsx`, `kanbanStore.js`, `projectSlice.js`, `boardSlice.js`, `cardSlice.js`, `useKanbanPermissions.js`, `kanbanConstants.js` | A1, B1-B4, C1.1-C1.3, **C1.7 Memory Leaks** |
| **4** | Frontend UI Components | `BoardView.jsx`, `KanbanList.jsx`, `KanbanCard.jsx`, `BoardSettingsDrawer.jsx`, `ProjectSettingsDrawer.jsx`, all Tabs/Reports/Workload/UserGuide | A1, B1, B6, C1.4-C1.7 |
