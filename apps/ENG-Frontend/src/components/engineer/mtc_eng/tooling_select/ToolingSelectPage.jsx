...
const ToolingSelectPage = () => {
  ...
  const [invKey, setInvKey] = useState('tsg300znc');
  ...
  const invTableConfig = toolingTables.find(t => t.key === invKey) || toolingTables[0];

  const fetchToolList = async (key) => {
    const cfg = toolingTables.find(t => t.key === key) || toolingTables[0];
    setInvLoading(true);
  ...
  const onAddTableSelect = async (tableName) => {
    ...
    setAddDimCols(Array.isArray(tbl?.dim_cols) ? tbl.dim_cols : []);
  ...
  const buildResults = (res) => {
    ...
    if (hasTsg) {
      ...
      mData.push({ name: 'TSG-300ZNC', group: 'FACE', found: zncFound, required: 2, tools: [
        { title: 'CHUTE',        content: <ToolingTable title="CHUTE"        dataSource={zncChutes}       columns={['valA','valB','valC','valD']} headers={['Slot Length (A)','Slot Height (B)','Min Width (C)','Center Slot (D)']} targets={[c?.chuteA,c?.chuteB,c?.chuteC,c?.chuteD]} icon={<SwapOutlined />} /> },
        { title: 'CARRIER (ZNC)',content: <ToolingTable title="CARRIER (ZNC)" dataSource={res.carriersZNC} columns={['valA','valB','valC','valD']} headers={['Slot Width (A)','ØPCD (B)','Slot Length (C)','Carrier Thin (D)']} targets={[c?.carrierA,c?.carrierB,c?.carrierC,'']} icon={<SettingOutlined />} /> },
      ]});
      const wFound = res.carriersW?.length ? 1 : 0;
      mData.push({ name: 'TSG300W', group: 'FACE', found: wFound, required: 1, tools: [
        { title: 'CHUTE',       content: <ToolingTable title="CHUTE"      dataSource={wChutes}       columns={['valA','valB','valC','valD']} headers={['Slot Length (A)','Slot Height (B)','Min Width (C)','Center Slot (D)']} targets={[c?.chuteA,c?.chuteB,c?.chuteC,c?.chuteD]} icon={<SwapOutlined />} /> },
        { title: 'CARRIER (W)', content: <ToolingTable title="CARRIER (W)" dataSource={res.carriersW} columns={['valA','valB']} headers={['Slot Width (A)','Carrier Thin (E)']} targets={[c?.carrierA,'']} icon={<SettingOutlined />} /> },
      ]});
    }
    ...
    if (ks && !ks.error) {
      ...
      mData.push({ name: 'KS400B', group: 'OD', found, required: 5, tools: [
        { title: 'WORK DRIVER',  content: <ToolingTable title="WORK DRIVER"  dataSource={res.ks400b.workDrivers}  columns={['val1','val2','val3','val4','val5','val6']} headers={['A (OD)','B (ID)','C (OD)','D (OD)','E (Width)','F (Step)']} targets={[ks.wd_A,ks.wd_B,ks.wd_C,ks.wd_D,ks.wd_E,ks.wd_F]} icon={<ToolOutlined />} /> },
        ...
        { title: 'PLUG A',        content: <ToolingTable title="PLUG A"        dataSource={res.ks400b.plugsA}        columns={['val1','val2','val3','val4','val5']} headers={['A (OD)','B (OD)','C (Depth)','D (Cham)','E (Dist)']} targets={[ks.pa_A,ks.pa_B,ks.pa_C,ks.pa_D,ks.pa_E]} icon={<NodeIndexOutlined />} /> },
        { title: 'PLUG B',        content: <ToolingTable title="PLUG B"        dataSource={res.ks400b.plugsB}        columns={['val1','val2','val3','val4','val5']} headers={['A (OD)','B (OD)','C (Depth)','D (Cham)','E (Dist)']} targets={[ks.pb_A,ks.pb_B,ks.pb_C,ks.pb_D,ks.pb_E]} icon={<NodeIndexOutlined />} /> },
      ]});
    }
    ...
    if (b5 && !b5.error) {
      ...
      mData.push({ name: 'KS400B5', group: 'OD', found, required: 10, tools: [
    ...
    if (b6 && !b6.error) {
      ...
      mData.push({ name: 'KS400B6', group: 'OD', found, required: 9, tools: [
    ...
    if (ks5 && !ks5.error) {
      ...
      mData.push({ name: 'KS500RD', group: 'OD', found, required: 3, tools: [
    ...
    if (ks3 && !ks3.error) {
      ...
      const ks3Tools = [
        ...
        { title: 'PLUG GAUGE',       content: <ToolingTable title="PLUG GAUGE"       dataSource={res.ks03a.plugGauges}     columns={['val1','val2','val3','val5']} headers={['A (ID)','B','C','Type']} targets={[ks3.pg?.A,ks3.pg?.B,ks3.pg?.C,ks3.pg?.Type]} icon={<NodeIndexOutlined />} /> },
        ...
        { title: 'MASTER RING GAUGE',content: <ToolingTable title="MASTER RING GAUGE" dataSource={res.ks03a.masterRings}   columns={['val1','val2','val3','val5']} headers={['A (OD)','B (F"A")','C (W)','Type']} targets={[ks3.mr?.A,ks3.mr?.B,ks3.mr?.C,'']} icon={<AuditOutlined />} /> },
        { title: 'CPX SHOE',         content: <ToolingTable title="CPX SHOE"         dataSource={res.ks03a.cpxShoes}       columns={['val1','val2','val3','val4','val5']} headers={['A (Width)','D (Height)','C','V / M','Type']} targets={[ks3.cpxShoe?.A,ks3.cpxShoe?.D,ks3.cpxShoe?.C,ks3.cpxShoe?.V,ks3.cpxShoe?.Type]} icon={<ToolOutlined />} /> },
        { title: 'LOADER',           content: <ToolingTable title="LOADER"           dataSource={res.ks03a.loader}         columns={['val1','val2','val3','val4','val5']} headers={['A (Width)','B (OD)','C','D','E']} targets={[`${ks3.ld?.A_min}-${ks3.ld?.A_max}`,ks3.ld?.B,ks3.ld?.C,ks3.ld?.D,ks3.ld?.E]} icon={<SwapOutlined />} /> },
        { title: 'ROLLER SHOE',      content: <ToolingTable title="ROLLER SHOE"      dataSource={res.ks03a.rollerShoes}    columns={['val1','val2','val3','val4','val5']} headers={['A (Height)','C (Width)','D','B','Type']} targets={[ks3.rollerShoe?.A,ks3.rollerShoe?.C,ks3.rollerShoe?.D,ks3.rollerShoe?.B,ks3.rollerShoe?.Type]} icon={<ToolOutlined />} /> },
      ];
    ...
    if (res.jaws?.length || res.bps?.length) {
      ...
      mData.push({ name: 'KS-B22G / KS-B80', group: 'ID', found, required: 2, tools: [
        { title: 'JAW',        content: <ToolingTable title="JAW"        dataSource={res.jaws} columns={['val1','val2','val3','valD','valE']} headers={['ØID (A)','ØID (B)','Min Width (C)','Max Depth (D)','E']} targets={[c.A,c.B,c.C,c.D_Limit,'']} icon={<ToolOutlined />} /> },
        { title: 'BACK PLATE', content: <ToolingTable title="BACK PLATE" dataSource={res.bps}  columns={['val1','val2']} headers={['ØID (A)','Min ØPCD (B)']} targets={[c.AA,c.BB]} icon={<BlockOutlined />} /> },
      ]});
    }
