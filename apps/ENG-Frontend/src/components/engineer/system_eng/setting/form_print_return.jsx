...
export const CollectionReturnForm = ({ onCancel, visiblePrint, printByDocNo }) => {
  ...
  useEffect(() => {
    const getData = async () => {
      if (visiblePrint) {
        const resultDocNo = await axios.get(
          server.ENG_TOOLING_INSP + "/" + printByDocNo
        );
        form.setFieldsValue({
          eng_docno: printByDocNo,
        });
        const x = sumQty(resultDocNo.data);
        // console.log(x);
        const addData = [];
        for (let i = 0; i < x.length; i++) {
          addData.push({
            key: x[i].key,
            item: i + 1,
            eng_docno: x[i].eng_docno,
            eng_itemno: x[i].eng_itemno,
            eng_spec: x[i].eng_spec,
            eng_return_qty: Number(x[i].eng_acc_qty) + Number(x[i].eng_rej_qty),
          });
        }
        setData(addData);
    ...
    getData();
  }, [form, printByDocNo, visiblePrint]);
  ...
  return (
    <Modal forceRender width={1000} open={visiblePrint} title="Tooling Return List" onCancel={onCancel} footer={footerModal}>
      <Form form={form} layout="vertical" name="form_in_modal" initialValues={{ modifier: "public" }}>
        ...
        <div id="tag_print" ref={componentRef}>
          ...
          <h5 className="text-center font-weight-bold">Rodend Bearing Division</h5>
          <font size="3">
            <table width="100%">
              <tbody>
                <tr>
            ...
            <table border="1px solid black" width="100%">
              ...
              <tbody>
                {data.map((x, index) => x.eng_return_qty > 0 ? (
