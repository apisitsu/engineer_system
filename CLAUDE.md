...
## Tooling Select (DB-driven)

Domain at `api/engineer/mtc/` (tsv2 files) — registered in `server.js` as `/api/tooling-select`. The legacy V1 tooling-select system has been retired. V1 files remain on disk under `api/engineer/mtc/` but are no longer routed.
...
- **ALL active rules** contribute to a single combined-distance `ORDER BY`:
