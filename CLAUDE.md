...
| **TI import** | `TI_INSP_REC_DIR`, `TI_DWG_PRINT_FILE`, `TI_CSV_OUTPUT_DIR`, `TI_CSV_SKIP_LOCAL` | Tooling Inspection "Update data". Source defaults are the two UNC shares. The CSVs reach Drive by the **browser** uploading them (frontend `GAS_TI_CSV_URL` — the Workspace blocks anonymous access to Apps Script, so the backend can't POST them). `TI_CSV_OUTPUT_DIR` (default `G:\Shared drives\...`) is now only a local backup; set `TI_CSV_SKIP_LOCAL=1` on prod, where the PM2 service account cannot see `G:`. Runbook → `api/engineer/mtc/doc/ti_csv_drive_upload_runbook.md` |
