...
const { poolTooling } = require('../db/pool_tooling');
const {
...
// Identity mapper — rows keyed by SQL-aliased Excel column names
const h = name => name;

async function findFixtures(cnNumber) {
  try {
    // 1. Get part data from spec_process
    const partResult = await poolTooling.query(
    ...
    // 3. Query all tooling tables in parallel (with SQL aliases for Excel column names)
    const [ksb22g, ksb80, tsg300, ks03a, ks400b, ks500rd, ks400b5, ks400b6] = await Promise.all([
      poolTooling.query(`
        SELECT tooling_name AS "Tooling_name", tooling_no AS "Tooling_no",
               jaw_id_1_a AS "JawID_1_A", jaw_id_2_b AS "JawID_2_B",
               jaw_width_max_c AS "JawWidth_Max_C", jaw_depth_max_d AS "JawDepth_Max_D",
               backplate_id_a AS "BackPlateID_A", backplate_pcd_b AS "BackPlatePCD_B",
      ...
      poolTooling.query(`
        SELECT tooling_name AS "Tooling_name", tooling_no AS "Tooling_no",
               ...
               jaw_e AS "Jaw_E",
               backplate_id_a AS "BackPlateID_A", backplate_pcd_b AS "BackPlatePCD_B",
               backplate_c AS "BackPlate_C", machine AS "Machine" FROM tooling_ksb80`),
      poolTooling.query(`
        SELECT tooling_name AS "Tooling_name", tooling_no AS "Tooling_no",
               face_chute_a AS "Face_Chute_A", face_chute_b AS "Face_Chute_B",
               face_chute_c AS "Face_Chute_C", face_chute_d AS "Face_Chute_D",
               face_carrier_a AS "Face_Carrier_A", face_carrier_b AS "Face_Carrier_B",
               face_carrier_c AS "Face_Carrier_C", face_carrier_d AS "Face_Carrier_D",
               face_carrier_e AS "Face_Carrier_E", face_carrier_f AS "Face_Carrier_F",
               face_carrier_g AS "Face_Carrier_G", machine AS "Machine" FROM tooling_tsg300`),
      poolTooling.query(`
        SELECT tooling_name AS "Tooling_name", tooling_no AS "Tooling_no",
      ...
      poolTooling.query(`
        SELECT tooling_name AS "Tooling_name", tooling_no AS "Tooling_no",
               od_a AS "OD_A", id_b AS "ID_B", od_c AS "OD_C", od_d AS "OD_D",
               width_e AS "Width_E", step_f AS "Step_F",
               loading_chute_a AS "LOADING_CHUTE_A", loading_chute_b AS "LOADING_CHUTE_B",
               loading_chute_c AS "LOADING_CHUTE_C", loading_chute_d AS "LOADING_CHUTE_D",
               loading_chute_e AS "LOADING_CHUTE_E", loading_chute_f AS "LOADING_CHUTE_F",
               support_block_a AS "SUPPORT_BLOCK_A", support_block_b AS "SUPPORT_BLOCK_B",
               support_block_c AS "SUPPORT_BLOCK_C", support_block_d AS "SUPPORT_BLOCK_D",
               support_block_e AS "SUPPORT_BLOCK_E",
               plug_a_od_a AS "PLUG_A_OD_A", plug_a_od_b AS "PLUG_A_OD_B",
               plug_a_depth_c AS "PLUG_A_DEPTH_C", plug_a_cham_d AS "PLUG_A_CHAM_D",
               plug_a_dist_e AS "PLUG_A_DIST_E",
               plug_b_od_a AS "PLUG_B_OD_A", plug_b_od_b AS "PLUG_B_OD_B",
               plug_b_depth_c AS "PLUG_B_DEPTH_C", plug_b_cham_d AS "PLUG_B_CHAM_D",
               plug_b_dist_e AS "PLUG_B_DIST_E",
    ...
    // 4. KSB22G — Machine Limit: OD ≤ 38, 4.8 ≤ ID < 16, W ≥ 14
    ...
    // 5. KSB80 — Machine Limit: 15 < OD ≤ 70, ID ≥ 7.9, W ≥ 14
    ...
    // 7. KS-03A — Machine Limit: OD_AFT ≤ 33
    let ks03a_results = {};
    ...
    // 9. KS500RD — Machine Limit: 14 ≤ ID ≤ 38.125, 26 ≤ OD ≤ 59.531, 23.5 ≤ W ≤ 51.15
    let ks500rd_results = {};
    ...
    // 10. KS400B5 — Machine Limit: OD ≤ 35
    let ks400b5_results = {};
